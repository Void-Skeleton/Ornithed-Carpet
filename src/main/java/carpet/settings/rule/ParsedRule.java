...
import carpet.settings.validation.Validator;
import carpet.settings.validation.Validators;
...
public final class ParsedRule<T> implements CarpetRule<T>, Comparable<ParsedRule<?>> {
	...
	public ParsedRule(Field field, Rule rule, SettingsManager settingsManager) {
		...
		if (categories.contains(RuleCategory.COMMAND)) {
			tempValidators.add(new Validator.CommandValidator<>());
		...
		if (this.isClient) {
			tempValidators.add(new Validator.ClientSettingValidator<>());
		}
