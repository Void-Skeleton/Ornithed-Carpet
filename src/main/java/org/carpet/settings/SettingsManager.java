...
//$$ 						executes((c) -> listSettings(c.getSource(), String.format(Translations.tr(TranslationKeys.ALL_MOD_SETTINGS), fancyName), getRulesSorted())).
//$$ 						then(CommandManager.argument("tag", StringArgumentType.word()).
//$$ 								suggests((c, b) -> suggest(getCategories(), b)).
//$$ 								executes((c) -> listSettings(c.getSource(), String.format(Translations.tr(TranslationKeys.MOD_SETTINGS_MATCHING), fancyName, RuleHelper.translatedCategory(identifier(),StringArgumentType.getString(c, "tag"))), getRulesMatching(StringArgumentType.getString(c, "tag")))))).
...
//$$ 	private int setDefault(CommandSourceStack source, CarpetRule<?> rule, String stringValue) {
	//#else
	public int setDefault(CommandSource source, CarpetRule<?> rule, String stringValue) {
	//#endif
		if (locked()) {
			return 0;
		}
		...
		try {
			...
			Messenger.m(source ,"gi "+String.format(Translations.tr(TranslationKeys.DEFAULT_SET), RuleHelper.translatedName(rule), stringValue));
...
//$$ 	private int removeDefault(CommandSourceStack source, CarpetRule<?> rule) {
	//#else
	public int removeDefault(CommandSource source, CarpetRule<?> rule) {
	//#endif
		if (locked) {
			return 0;
		}
		...
		Messenger.m(source ,"gi "+String.format(Translations.tr(TranslationKeys.DEFAULT_REMOVED), RuleHelper.translatedName(rule)));
		return 1;
	}
...
//$$ 	public void inspectClientsideCommand(CommandSourceStack source, String string) {
	...
	public boolean matchesSubStr(String string, String string2) {
...
//$$ 	private int setRule(CommandSourceStack source, CarpetRule<?> rule, String newValue) {
	//#else
	public int setRule(CommandSource source, CarpetRule<?> rule, String newValue) {
	//#endif
		try {
			rule.set(source, newValue);
			Messenger.m(source, "w "+ rule +", ", "c ["+ Translations.tr(TranslationKeys.CHANGE_PERMANENTLY)+"?]",
					"^w "+String.format(Translations.tr(TranslationKeys.CHANGE_PERMANENTLY_HOVER), identifier+".conf"),
...
//$$ 	private CarpetRule<?> contextRule(String ruleName) throws CommandSyntaxException {
	//#else
	public CarpetRule<?> contextRule(String ruleName) {
	//#endif
		//#if MC>=11300
//$$ 		CarpetRule<?> rule = getCarpetRule(ruleName);
//$$ 		if (rule == null) {
//$$ 			throw new SimpleCommandExceptionType(Messenger.c("rb " + Translations.tr(TranslationKeys.UNKNOWN_RULE) + ": " + ruleName)).create();
//$$ 		}
//$$ 		return rule;
		//#else
...
//$$ 	private int displayRuleMenu(CommandSourceStack source, CarpetRule<?> rule) {  // TODO check if there's dupe code around options buttons
	//#else
	public int displayRuleMenu(CommandSource source, CarpetRule<?> rule) {  // TODO check if there's dupe code around options buttons
	//#endif
		...
		tags.add(Messenger.c("w "+ Translations.tr(TranslationKeys.TAGS)+": "));
		for (String t: rule.categories()) {
			String translated = RuleHelper.translatedCategory(identifier(), t);
			tags.add(Messenger.c("c ["+ translated +"]", "^g "+ String.format(Translations.tr(TranslationKeys.LIST_ALL_CATEGORY), translated),"!/"+identifier+" list "+t));
			tags.add(Messenger.c("w , "));
		}
		tags.remove(tags.size() - 1);
		Messenger.m(source, tags.toArray(new Object[0]));

		Messenger.m(source, "w "+ Translations.tr(TranslationKeys.CURRENT_VALUE)+": ", String.format("%s %s (%s value)", RuleHelper.getBooleanValue(rule) ? "lb" : "nb", RuleHelper.toRuleString(rule.value()), RuleHelper.isInDefaultValue(rule) ? "default" : "modified"));
	...
	public Collection<CarpetRule<?>> getRulesMatching(String search) {
...
//$$ 	public int listAllSettings(CommandSourceStack source) {
	...
	//#endif
		int count = listSettings(source, String.format(Translations.tr(TranslationKeys.CURRENT_SETTINGS_HEADER), fancyName), getNonDefault());

		if (version != null) {
			Messenger.m(source, "g " + fancyName + " " + Translations.tr(TranslationKeys.VERSION) + ": " + version);
		}

		List<String> tags = new ArrayList<>();
		tags.add("w " + Translations.tr(TranslationKeys.BROWSE_CATEGORIES)  + ":\n");
		for (String t : this.getCategories()) {
			...
			tags.add("^g " + String.format(Translations.tr(TranslationKeys.LIST_ALL_CATEGORY), translatedPlus));
	...
	public Collection<CarpetRule<?>> getRulesSorted() {
...
//$$ 	private int listSettings(CommandSourceStack source, String title, Collection<CarpetRule<?>> settings_list) {
	//#else
	public int listSettings(CommandSource source, String title, Collection<CarpetRule<?>> settings_list) {
	//#endif
	...
	private Text makeSetRuleButton(CarpetRule<?> rule, String option, boolean brackets) {
		...
		return Messenger.c(component, "^g "+ String.format(Translations.tr(TranslationKeys.SWITCH_TO), option + (option.equals(RuleHelper.toRuleString(rule.defaultValue())) ? " (default)" : "")), "?/" + identifier + " " + rule.name() + " " + option);
	}
